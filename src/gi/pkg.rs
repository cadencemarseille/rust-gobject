...
fn do_install(args: ~[~str]) {
    ...
    let gi_libs = match os::getenv("GOBJECT_INTROSPECTION_LIBS") {
        None => {
            let pkg_config_output = run::process_output("pkg-config", [~"--libs", ~"gobject-introspection-1.0"]);
            if pkg_config_output.status != 0 {
                fail!("Package script error: `pkg-config --libs gobject-introspection-1.0` failed");
            }
            ...
            os::setenv("GOBJECT_INTROSPECTION_LIBS", libs_str);
            libs_str
        },
        Some(gi_libs) => gi_libs
    };
    // `pkg-config` adds a newline to the end, which we need to trim away because newlines
    // in link_args cause build issues.
    let trimmed_gi_libs = gi_libs.trim();
    debug!("GOBJECT_INTROSPECTION_LIBS=\"%s\"", trimmed_gi_libs);
        ...
        match path_to_substitute.filestem() {
            ...
            Some(filestem) => {
                ...
                do r.each_line |line| -> bool {
                    let substituted_line = str::replace(line, "@GOBJECT_INTROSPECTION_LIBS@", trimmed_gi_libs);
    ...
    api::build_lib(sysroot_path, workspace_path, ~"gi", rustpkg::version::ExactRevision(~"0.1"), Path("mod.rs"));
}
